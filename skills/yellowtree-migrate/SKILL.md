...
- **Template author-markers (`trend3` text color)**: the template's legal pages mark every adapt-per-site passage (operator address, DPO, "last updated", TODO paragraphs) with `"textColor":"trend3"` + `has-trend-3-color has-text-color has-link-color` classes — it renders as colored text on the live site, it is NOT design. ASK the owner directly when building legal pages: keep the markers as a work aid until review is done, or strip them now (default once content is adapted: STRIP — remove the textColor/style.elements.link attrs AND the matching classes together). Warn that stripped TODO placeholders lose their visual highlight — list them in the manifest so they aren't forgotten.
...
## Vision page (auto-create from the Movement)

Every ICF site should carry the Movement's vision page. Don't source it from the old site — fetch the CANONICAL latest version live from https://icf.church/en/vision/ (rendered Yellowtree page), reconstruct the saved block markup per the reference, keep the EN content 1:1 (official Movement content — the site-content whitelist does not apply to this page), and translate faithfully into the site's other locales. Point CTAs at the site's own pages (site-relative) where local equivalents exist; keep pure Movement links absolute. Note in the manifest: images are Movement-site assets (re-upload locally before go-live) and the page should be re-fetched whenever the Movement updates it.
- **Core-block spacing (heading/paragraph): NEVER strip template spacing.** The valid editor form is the PAIR — JSON attr (`"style":{"spacing":{"margin":{...}}}`) AND the matching inline `style="margin-..."` on the tag together. Attr without inline style breaks block validation; stripping both kills the page rhythm (titles glued to heroes, no gap before the footer). Copy spacing 1:1 from the template pages.

## Dynamic widgets — post-paste configuration (editor-only, not in markup)

Dynamic blocks render errors until configured IN THE EDITOR after pasting: `services-list` ("no service widget selected" → pick e.g. All Services), `events-list`/`featured-events` (need an org-level Events API key — request from platform support if "no API key for this organization"; note WPML makes theme options language-specific, so the key may exist for one language and be missing for another), `sermons-list`, `gravityforms/form` with `formId:"TODO"` (create the form, reassign the block; one GF form serves all WPML languages). TWO RULES: (1) migrate-all's final report MUST list every dynamic widget per page as a configuration checklist; (2) order matters — paste final content FIRST, configure widgets AFTER: re-pasting page code WIPES widget configuration.
